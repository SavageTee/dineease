...
adminApi.get('/gethotels', csrfProtection, async (req:Request, res:Response, next:NextFunction):Promise<any>=>{
  try{
    ...
    const updatedData = (result as any)[0].map((item:any)=> {
      return {
          ...item, 
          verification_text: item.verification_type === 0 ? i18next.t('birthDay',{ ns:'admin_hotels_page', lng:req.language }) : i18next.t('departureDate',{ ns:'admin_hotels_page', lng:req.language })
      };
    });
